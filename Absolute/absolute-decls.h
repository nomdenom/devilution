...
void __fastcall d_CPrintString(int No,unsigned int glyph,unsigned char col);
...
void __fastcall d_j_lock_buf_priv(BYTE idx);
void __cdecl d_lock_buf_priv();
void __fastcall d_j_unlock_buf_priv(BYTE idx);
...
void __fastcall d_priv_sound_init(UCHAR bLoadMask);
...
void __fastcall d_WSave(int v);
void __fastcall d_OSave(BOOL v);
...
BOOL __fastcall d_mpqapi_write_file(const char * pszName,const BYTE * pbData,DWORD dwLen);
_BLOCKENTRY * __fastcall d_mpqapi_add_file(const char * pszName,_BLOCKENTRY * pBlk,int block_index);
bool __fastcall d_mpqapi_write_file_contents(const char * pszName,const BYTE * pbData,int dwLen,_BLOCKENTRY * pBlk);
...
void d_pfile_cpp_init(...); // Unknown signature
...
BOOL __fastcall d_pfile_create_player_description(char * dst,unsigned int len);
BOOL __fastcall d_pfile_create_save_file(const char * name_1,const char * name_2);
...
BOOL __fastcall d_pfile_read_hero(HANDLE archive,PkPlayerStruct * pPack);
...
BOOL __fastcall d_pfile_archive_contains_game(HANDLE hsArchive,unsigned int save_num);
BOOL __stdcall d_pfile_ui_set_class_stats(unsigned int player_class_nr,_uidefaultstats * class_stats);
...
void __fastcall d_pfile_write_save_file(const char * pszName,BYTE * pbData,DWORD dwLen,DWORD qwLen);
void __fastcall d_pfile_strcpy(char * dst,const char * src);
BYTE * __fastcall d_pfile_read(const char * pszName,DWORD * pdwLen);
...
void __fastcall d_plrmsg_delay(BOOL delay);
char * __fastcall d_ErrorPlrMsg(const char * pszMsg);
size_t d_EventPlrMsg(const char * pszFmt,...);
...
void __fastcall d_PrintPlrMsg(unsigned int x,unsigned int y,unsigned int width,const char * str,unsigned char just);
...
int __fastcall d_tmsg_get(BYTE * pbMsg,DWORD dwMaxLen);
void __fastcall d_tmsg_add(BYTE * pbMsg,BYTE bLen);
